<template>
    <div>
      <div style="padding: 10px 0">
        <el-input placeholder="请输入查询内容" v-model="input" class="input-with-select" style="width: 500px" clearable>
          <el-select v-model="select" slot="prepend" placeholder="请选择" clearable>
            ...
            <el-option label="名称" value="categoryName"></el-option>
          </el-select>
      ...
      <el-table :data="tableData" border stripe :header-cell-style="{'text-align':'center'}"
        ...
        <el-table-column prop="description" label="分类简介">
        </el-table-column>
        <el-table-column
            label="操作">
          <template slot-scope="scope">
      ...
      <el-dialog title="新增或修改标签(请单击确定保存)" :visible.sync="dialogvisible" width="30%">
        <el-form :model="form" status-icon  ref="form" :rules="rules">
          ...
          <el-form-item label="分类简介" :label-width="formLabelWidth" prop="description">
...
export default {
  ...
  data() {
    return {
      ...
      tags:[
          {value: '/static/category/back.png'},
        {value: '/static/category/database.png'},
        {value: '/static/category/front.png'},
        {value: '/static/category/language.png'},
        {value: '/static/category/lift.jpg'},
        {value: '/static/category/star.png'}
      ],
      rules: {
        categoryName: [
