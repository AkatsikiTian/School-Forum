          ...
          <el-form-item label="文章分类" prop="category">
            <el-select v-model="articleForm.category" value-key="id" placeholder="请选择文章分类">
              <div v-for="c in categorys" :key="c.id" >
                <el-option :label="c.categoryName" :value="c" v-if="c.categoryName.includes('精品贴')&&$store.state.admin==1"></el-option>
                <el-option :label="c.categoryName" :value="c" v-if="!c.categoryName.includes('精品贴')"></el-option>
              </div>
        ...
        this.$refs[articleForm].validate((valid) => {
          if (valid) {
            ...
            publishArticle(article,this.$store.state.token).then((data) => {
              if(data.success){
                loading.close();
                that.$message({message: '文章提交成功啦', type: 'success', showClose: true})
