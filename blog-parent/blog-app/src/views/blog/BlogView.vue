      ...
      <el-main>
        ...
        <div class="me-view-card">
          ...
          <div class="me-view-author">
            <a class="">
              <img class="me-view-picture" :src="article.author_avatar"></img>
            </a>
            ...
            <el-button
              v-if="this.article.author_id == this.$store.state.id"
              @click="editArticle()"
            ...
            <el-button
              v-if="this.article.author_id == this.$store.state.id"
              @click="deleteArticle();deleteArticleBody();deleteArticleTag();deleteArticleComment()"
...
<script>
  ...
  import {deleteArticleBody, deleteArticleById, deleteArticleComment, deleteArticleTag} from "../../api/article";
  ...
  export default {
    ...
    computed: {
      ...
      title() {
        return `${this.article.title} - 文章 - sft3`
      }
    },
    methods: {
      ...
      deleteArticle(){
        let that = this
        deleteArticleById(that.article.id).then(data=>{
          console.log(data)
          that.$message({type: 'success', message: '删除成功', showClose: true})
          that.$router.push({path: '/'})
        }).catch(error => {
          if (error !== 'error') {
            that.$message({type: 'error', message: '删除加载失败', showClose: true})
          }
        })
      },
      deleteArticleComment(){
        let that = this
        deleteArticleComment(that.article.id).then(data=>{
          console.log(data)
        }).catch(error => {
          if (error !== 'error') {
            that.$message({type: 'error', message: '删除加载失败', showClose: true})
      ...
      deleteArticleBody(){
        let that = this
        deleteArticleBody(that.article.id).then(data=>{
          console.log(data)
        }).catch(error => {
          if (error !== 'error') {
            that.$message({type: 'error', message: '删除加载失败', showClose: true})
      ...
      deleteArticleTag(){
        let that = this
        deleteArticleTag(that.article.id).then(data=>{
          console.log(data)
        }).catch(error => {
          if (error !== 'error') {
            that.$message({type: 'error', message: '删除加载失败', showClose: true})
